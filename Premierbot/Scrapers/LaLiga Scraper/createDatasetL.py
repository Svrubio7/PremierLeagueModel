...
def fetch_match_statistics(match_id: int, max_retries=3):
    """Fetch match statistics with retry logic and random delays"""
    for attempt in range(max_retries):
        try:
            ...
            resp = requests.get(
                url, 
                headers=HEADERS,
                impersonate="chrome110",  # Use curl_cffi's browser impersonation
                timeout=30
            )
            ...
            if attempt == max_retries - 1:
                print(f"Failed after {max_retries} attempts for match {match_id}: {e}")
                raise
            print(f"Attempt {attempt + 1} failed, retrying after longer delay...")
